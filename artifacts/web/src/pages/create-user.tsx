...
interface AcademicItem {
  id: string;
  type: 'section' | 'year_level' | 'semester' | 'school_year';
  label: string;
  sortOrder: number;
  isActive: boolean;
}
...
function useAcademicItems(type: AcademicItem['type']) {
...
const baseSchema = z.object({
  ...
  // Student fields (studentNumber is auto-generated server-side, not user input)
...
export function CreateUserPage() {
  ...
  const { data: schoolYears = [] } = useAcademicItems('school_year');

  const form = useForm<FormData>({
    resolver: zodResolver(baseSchema),
    defaultValues: {
      firstName: '', lastName: '', email: '', password: '', confirmPassword: '',
      role: 'student', phone: '', section: '',
      program: 'BSN', academicYear: '2026-2027', employeeId: '', specialization: '',
    },
  });
              ...
              <CardContent className="space-y-4">
                <div className="rounded-md border bg-muted/40 px-3 py-2">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Automatically generated by the system when the account is created.
                  </p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="academicYear" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Managed under Academic Settings.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
